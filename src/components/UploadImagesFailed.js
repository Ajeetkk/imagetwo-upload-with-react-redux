...
class UploadImagesFailed extends Component{
  ...
  handleClick(e) {
    ...
    reader.onload = () => {
     let itemArray = this.state.selectImage;
     let item = {}
     item.name = this.state.productimage[i].name
     item.type = this.state.productimage[i].type
     item.image = reader.result
     itemArray[i] = item
     this.setState({
        file: reader.result,
        selectImage:itemArray
      })
      this.refs['selectImage'+i].src=reader.result;
    };
...
onChangeFile(e) {
  ...
  var file = e.target.files[0]
    ...
    reader.readAsDataURL(file)
    let item = {}
    item.name = this.state.productimage[0].name
    item.type = this.state.productimage[0].type
    ...
    //alert( reader.result)
      this.setState({
        file: reader.result,
        selectImage:this.state.selectImage.concat([item])
      })
      this.refs['selectImage'+0].src= reader.result;
    };
    reader.onerror = function (error) {
      console.log('Error: ', error);
    }
       ...
       //let data = this.props.selectImage && JSON.parse(this.props.selectImage) 
        ...
        <Grid>
          ...
          <Col md={4} xsHidden smHidden></Col>
            <Col md={4} xsHidden smHidden style={{background:'white'}}>
           <div style={{backgroundImage: `url(require("../resources/top-bar-bg@3x.png"))`, height:'20px'}}></div>
       ...
       <ul style={{listStyle: "none"}}>
          ...
          //   :
            <div>
               ...
               { data[i].uploaded?
                  <Button style={{position:'absolute', zIndex: '3', background:'green', marginTop: '20px'}}>
                  <img src={require('../resources/ok.png')} responsive/>
                </Button>
                :
                  <Button style={{position:'absolute', zIndex: '3', background:'red', marginTop: '20px'}}>
